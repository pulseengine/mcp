//! Hello World MCP Server Example with Streamable HTTP
//!
...
impl McpBackend for HelloWorldBackend {
    ...
    fn get_server_info(&self) -> ServerInfo {
        ServerInfo {
            ...
            instructions: Some(
                "A simple demonstration server with basic greeting functionality using streamable-http transport".to_string(),
            ),
        }
    }
...
async fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    info!("🚀 Starting Hello World MCP Server with Streamable HTTP");

    // Create backend
    let backend_config = HelloWorldConfig::default();
    let backend = HelloWorldBackend::initialize(backend_config).await?;

    // Create server configuration with streamable-http transport
    let server_config = ServerConfig {
        ...
        auth_config: AuthConfig {
            storage: StorageConfig::Memory, // Use memory storage to avoid file issues
    ...
    info!("✅ Hello World MCP Server started successfully on port 3002");
    info!("💡 Available tools: say_hello, count_greetings");
    info!("🔗 Connect using MCP Inspector:");
