...
impl Default for EnhancedHelloWorldServer {
    fn default() -> Self {
        ...
        templates.insert("formal".to_string(), "Good day, {name}. I hope this message finds you well.".to_string());
        templates.insert("casual".to_string(), "Hey {name}! What's up? 😊".to_string());
        templates.insert("enthusiastic".to_string(), "WOW! Hi there {name}! So excited to meet you! 🎉".to_string());
        templates.insert("professional".to_string(), "Dear {name}, thank you for connecting with our service.".to_string());
        templates.insert("friendly".to_string(), "Hi {name}! Nice to meet you! 🤝".to_string());
...
impl EnhancedHelloWorldServer {
    ...
    pub async fn say_hello(
        ...
        let template = templates.get(&greeting_type)
    ...
    pub fn get_greeting_stats(&self) -> serde_json::Value {
        ...
        for record in history.iter() {
            ...
            *greeting_type_counts.entry(greeting_type.to_string()).or_insert(0) += 1;
        }
...
async fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    tracing::info!("📚 Documentation: This server demonstrates the full power of PulseEngine MCP macros");
