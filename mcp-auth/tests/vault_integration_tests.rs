...
mod vault_tests {
    ...
    fn test_vault_config_creation() {
        ...
        assert_eq!(config.base_url, Some("https://app.infisical.com".to_string()));
    ...
    fn test_vault_config_customization() {
        ...
        assert_eq!(config.base_url, Some("https://custom.infisical.com".to_string()));
    ...
    fn test_vault_types() {
        ...
        assert_eq!(VaultType::AWSSecretsManager.to_string(), "AWS Secrets Manager");
        assert_eq!(VaultType::Azure.to_string(), "Azure Key Vault");
        assert_eq!(VaultType::GoogleSecretManager.to_string(), "Google Secret Manager");
        assert_eq!(VaultType::Custom("Test".to_string()).to_string(), "Custom: Test");
    }
    ...
    async fn test_vault_secret_operations() {
        ...
        match VaultIntegration::new(config).await {
            Ok(integration) => {
                ...
                match integration.get_secret_cached("PULSEENGINE_MCP_SESSION_TIMEOUT").await {
                    Ok(value) => {
