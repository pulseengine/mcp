...
mod tests {
    ...
    fn test_consent_type_display() {
        ...
        assert_eq!(ConsentType::Custom("Special Processing".to_string()).to_string(), "Custom: Special Processing");
    }
    
    #[test]
    fn test_legal_basis_display() {
        assert_eq!(LegalBasis::Consent.to_string(), "Consent (GDPR 6.1.a)");
        assert_eq!(LegalBasis::LegitimateInterests.to_string(), "Legitimate Interests (GDPR 6.1.f)");
    }
    ...
    fn test_data_categories() {
        ...
        assert!(record.data_categories.contains(&"personal_identifiers".to_string()));
        assert!(record.data_categories.contains(&"authentication_data".to_string()));
    }
}