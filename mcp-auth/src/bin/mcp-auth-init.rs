...
use pulseengine_mcp_auth::{
    ...
    ValidationConfig, RoleRateLimitConfig,
...
async fn main() {
    ...
    let result = match cli.command {
        Some(Commands::Setup { expert }) => {
            run_setup_wizard(&cli, expert).await
        }
        ...
        Some(Commands::Migrate { ref from }) => {
            run_migration(&cli, from.clone()).await
        }
        None => {
...
async fn run_setup_wizard(cli: &Cli, expert_mode: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if validation.has_secure_random {
        println!("  {} Secure random number generation available", "✓".green());
    } else {
    ...
    if let Some(output_path) = &cli.output {
        ...
        println!();
        println!("{} Configuration saved to: {}", "✓".green(), output_path.display());
    } else {
        println!();
...
fn configure_quick_setup(mut builder: SetupBuilder) -> Result<SetupBuilder, Box<dyn std::error::Error>> {
    // Check for existing master key
    if std::env::var("PULSEENGINE_MCP_MASTER_KEY").is_ok() {
        builder = builder.with_env_master_key()?;
        println!("  {} Using existing master key from environment", "✓".green());
    } else {
...
async fn configure_custom_setup(
    ...
    let storage_types = vec!["Encrypted File Storage", "Environment Variables", "Custom Path"];
...
async fn configure_security_settings(theme: &ColorfulTheme) -> Result<ValidationConfig, Box<dyn std::error::Error>> {
...
async fn run_validation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  OS Support: {}", 
        if validation.os_supported { "✓ Supported".green() } else { "✗ Not Supported".red() }
    );
    println!("  Secure Random: {}", 
        if validation.has_secure_random { "✓ Available".green() } else { "✗ Not Available".red() }
    );
    println!("  Write Permissions: {}", 
        if validation.has_write_permissions { "✓ Available".green() } else { "⚠ Limited".yellow() }
    );
    println!("  Keyring Support: {}", 
        if validation.has_keyring_support { "✓ Available".green() } else { "⚠ Not Available".yellow() }
    );
...
async fn run_migration(_cli: &Cli, from: PathBuf) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!();
    println!("{} Migration functionality not yet implemented.", "⚠".yellow());
    println!("Please use manual setup for now.");
...
fn show_post_setup_instructions(result: &pulseengine_mcp_auth::setup::SetupResult) {
    ...
    println!("   {}", format!("export PULSEENGINE_MCP_MASTER_KEY={}", result.master_key).bright_black());
    println!();
    ...
    println!("   {}", "mcp-auth-cli audit query --limit 10".bright_black());
    println!();
