...
use crate::{AuthContext, jwt::{JwtManager, JwtConfig, JwtError}};
...
impl SessionStorage for MemorySessionStorage {
    async fn store_session(&self, session: &Session) -> Result<(), SessionError> {
        ...
        debug!("Stored session {} for user {}", session.session_id, session.user_id);
        Ok(())
    }
...
impl SessionManager {
    ...
    pub async fn create_session(
        ...
        let jwt_token = if let Some(jwt_manager) = &self.jwt_manager {
            let token = jwt_manager.generate_access_token(
                session.auth_context.user_id.clone().unwrap_or_else(|| user_id.clone()),
    ...
    pub async fn validate_jwt_token(&self, token: &str) -> Result<AuthContext, SessionError> {
        let jwt_manager = self.jwt_manager.as_ref()
    ...
    pub async fn refresh_session(
        ...
        let stored_refresh_token = session.refresh_token.as_ref()
            .ok_or_else(|| SessionError::SessionInvalid {
    ...
    pub async fn terminate_user_sessions(&self, user_id: &str) -> Result<u64, SessionError> {
        ...
        for session in sessions {
            if self.storage.delete_session(&session.session_id).await.is_ok() {
                terminated_count += 1;
            }
        }
    ...
    pub async fn start_cleanup_task(&self) -> tokio::task::JoinHandle<()> {
        ...
        tokio::spawn(async move {
            let mut cleanup_interval = tokio::time::interval(interval.to_std().unwrap_or(std::time::Duration::from_secs(3600)));
...
mod tests {
    ...
    async fn test_session_limits() {
        ...
        let result3 = manager.create_session(
            "test_user".to_string(),
            auth_context,
            None,
            None,
        ...
        assert!(matches!(result3, Err(SessionError::MaxSessionsExceeded { .. })));
    }
    ...
    async fn test_cleanup_expired_sessions() {
        ...
        let (_, _) = manager.create_session(
            "test_user".to_string(),
            auth_context,
            Some(chrono::Duration::milliseconds(1)),
            None,
