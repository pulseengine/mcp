...
mod tests {
    ...
    fn test_monitoring_module_exports() {
        ...
        let action = AlertAction::Log { level: "info".to_string() };
    ...
    fn test_alert_thresholds() {
        let thresholds = vec![
            ...
            AlertThreshold::Rate { count: 10, duration: chrono::Duration::minutes(5) },
            AlertThreshold::Percentage { 
                numerator_events: vec![SecurityEventType::AuthFailure],
                denominator_events: vec![SecurityEventType::AuthSuccess, SecurityEventType::AuthFailure],
                threshold: 50.0,
            },
        ];
        ...
        for threshold in thresholds {
            match threshold {
                ...
                AlertThreshold::Percentage { threshold: percentage, .. } => {
    ...
    fn test_alert_actions() {
        let actions = vec![
            AlertAction::Log { level: "warn".to_string() },
            AlertAction::Email { recipients: vec!["admin@example.com".to_string()] },
            AlertAction::Webhook { 
    ...
    fn test_monitoring_error_types() {
        let errors = vec![
            MonitoringError::AlertNotFound { alert_id: "test-alert".to_string() },
            MonitoringError::MetricNotFound { metric_name: "test-metric".to_string() },
            MonitoringError::ConfigError { reason: "test config error".to_string() },
    ...
    async fn test_security_dashboard_integration() {
        ...
        // Record some events
        let events = vec![
            SecurityEvent::new(SecurityEventType::AuthSuccess, SecuritySeverity::Low, "Auth 1".to_string()),
            SecurityEvent::new(SecurityEventType::AuthSuccess, SecuritySeverity::Low, "Auth 2".to_string()),
            SecurityEvent::new(SecurityEventType::AuthFailure, SecuritySeverity::Medium, "Failed auth".to_string()),
            SecurityEvent::new(SecurityEventType::RateLimit, SecuritySeverity::High, "Rate limit".to_string()),
        ];
