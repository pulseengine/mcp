...
use super::{SecretMetadata, VaultClient, VaultClientInfo, VaultConfig, VaultError, VaultType};
...
impl InfisicalClient {
    ...
    pub async fn new(config: VaultConfig) -> Result<Self, VaultError> {
        ...
        let client_id = std::env::var("INFISICAL_UNIVERSAL_AUTH_CLIENT_ID").map_err(|_| {
        ...
        let client_secret =
            std::env::var("INFISICAL_UNIVERSAL_AUTH_CLIENT_SECRET").map_err(|_| {
                VaultError::ConfigError(
                    "INFISICAL_UNIVERSAL_AUTH_CLIENT_SECRET environment variable not set"
                        .to_string(),
                )
            })?;
        ...
        let secret_path =
            std::env::var("INFISICAL_SECRET_PATH").unwrap_or_else(|_| "/".to_string());

        let client = Client::builder()
            .timeout(std::time::Duration::from_secs(config.timeout_seconds))
            .build()
            .map_err(|e| {
                VaultError::NetworkError(format!("Failed to create HTTP client: {}", e))
            })?;
    ...
    async fn get_access_token(&self) -> Result<String, VaultError> {
        ...
        token_info
            ...
            .ok_or_else(|| {
                VaultError::AuthenticationFailed("Failed to obtain access token".to_string())
            })
    }
...
impl VaultClient for InfisicalClient {
    async fn authenticate(&self) -> Result<(), VaultError> {
        ...
        let response = self
            ...
            .await
            .map_err(|e| {
                VaultError::NetworkError(format!("Authentication request failed: {}", e))
            })?;
        ...
        let auth_response: AuthResponse = response.json().await.map_err(|e| {
            VaultError::InvalidResponse(format!("Failed to parse auth response: {}", e))
        })?;
        ...
        let expires_at =
            std::time::Instant::now() + std::time::Duration::from_secs(auth_response.expires_in);
    ...
    async fn get_secret_with_metadata(
        ...
        if !response.status().is_success() {
            ...
            return Err(VaultError::NetworkError(format!(
                "Secret request failed with status {}: {}",
                status, error_text
            )));
        }

        let secret_response: SecretResponse = response.json().await.map_err(|e| {
            VaultError::InvalidResponse(format!("Failed to parse secret response: {}", e))
        })?;
        ...
        if !response.status().is_success() {
            ...
            if status == 401 {
                ...
                return Err(VaultError::AccessDenied(
                    "Access denied to list secrets".to_string(),
                ));
            }

            return Err(VaultError::NetworkError(format!(
                "List secrets failed with status {}: {}",
                status, error_text
            )));
        }

        let secrets_response: SecretsListResponse = response.json().await.map_err(|e| {
            VaultError::InvalidResponse(format!("Failed to parse secrets list response: {}", e))
        })?;

        let secret_names: Vec<String> = secrets_response
            .secrets
            .into_iter()
            .map(|secret| {
    ...
    async fn delete_secret(&self, name: &str) -> Result<(), VaultError> {
        ...
        let response = self
            ...
            .await
            .map_err(|e| {
                VaultError::NetworkError(format!("Delete secret request failed: {}", e))
            })?;
        ...
        if !response.status().is_success() {
            ...
            if status == 401 {
                ...
                return Err(VaultError::AccessDenied(format!(
                    "Access denied to delete secret '{}'",
                    name
                )));
            }

            return Err(VaultError::NetworkError(format!(
                "Delete secret failed with status {}: {}",
                status, error_text
            )));
        }
...
impl InfisicalClient {
    ...
    pub async fn set_secret_with_comment(
        &self,
        name: &str,
        value: &str,
        comment: Option<&str>,
        ...
        let workspace_id = self.workspace_id.as_ref().ok_or_else(|| {
            VaultError::ConfigError("Workspace ID required for creating secrets".to_string())
        })?;
        ...
        let response = self
            ...
            .await
            .map_err(|e| {
                VaultError::NetworkError(format!("Create secret request failed: {}", e))
            })?;
        ...
        if !response.status().is_success() {
            ...
            if status == 401 {
                ...
                return Err(VaultError::AccessDenied(format!(
                    "Access denied to create secret '{}'",
                    name
                )));
            }

            return Err(VaultError::NetworkError(format!(
                "Create secret failed with status {}: {}",
                status, error_text
            )));
        }
    ...
    pub async fn get_secret_version(
        ...
        debug!(
            "Fetching secret '{}' version {} from Infisical",
            name, version
        );
        ...
        if !response.status().is_success() {
            ...
            if status == 401 {
                ...
                return Err(VaultError::AccessDenied(format!(
                    "Access denied to secret '{}' version {}",
                    name, version
                )));
            }
        ...
        debug!(
            "Successfully retrieved secret '{}' version {}",
            name, version
        );
        Ok((secret.secret_value, metadata))
    }
