...
use super::auth_extractors::{
    AuthExtractor, AuthExtractionResult, TransportAuthContext, TransportRequest, 
    TransportType, TransportAuthError, AuthUtils
};
...
impl HttpAuthExtractor {
    ...
    fn extract_authorization_header(&self, headers: &HashMap<String, String>) -> AuthExtractionResult {
        ...
        // Try Bearer token
        if auth_header.starts_with("Bearer ") && self.config.supported_methods.contains(&HttpAuthMethod::Bearer) {
            match AuthUtils::extract_bearer_token(auth_header) {
                Ok(token) => {
                    AuthUtils::validate_api_key_format(&token)?;
                    let context = TransportAuthContext::new(token, "Bearer".to_string(), TransportType::Http);
        ...
        if auth_header.starts_with("Basic ") && self.config.supported_methods.contains(&HttpAuthMethod::Basic) {
            return self.extract_basic_auth(auth_header);
        }
        
        Err(TransportAuthError::InvalidFormat(
            format!("Unsupported Authorization header format: {}", auth_header)
        ))
    }
    ...
    fn extract_basic_auth(&self, auth_header: &str) -> AuthExtractionResult {
        if !auth_header.starts_with("Basic ") {
            return Err(TransportAuthError::InvalidFormat("Invalid Basic auth format".to_string()));
        }
        ...
        let decoded = match general_purpose::STANDARD.decode(encoded) {
            Ok(bytes) => match String::from_utf8(bytes) {
                Ok(string) => string,
                Err(_) => return Err(TransportAuthError::InvalidFormat("Invalid UTF-8 in Basic auth".to_string())),
            },
            Err(_) => return Err(TransportAuthError::InvalidFormat("Invalid Base64 in Basic auth".to_string())),
        };
        
        let parts: Vec<&str> = decoded.splitn(2, ':').collect();
        if parts.len() != 2 {
            return Err(TransportAuthError::InvalidFormat("Basic auth must be username:password".to_string()));
        }
        ...
        let context = TransportAuthContext::new(api_key.to_string(), "Basic".to_string(), TransportType::Http);
        Ok(Some(context))
    }
    ...
    fn extract_api_key_header(&self, headers: &HashMap<String, String>) -> AuthExtractionResult {
        ...
        if let Some(api_key) = AuthUtils::extract_api_key_header(headers) {
            AuthUtils::validate_api_key_format(&api_key)?;
            let context = TransportAuthContext::new(api_key, "X-API-Key".to_string(), TransportType::Http);
            return Ok(Some(context));
        }
    ...
    fn extract_query_auth(&self, request: &TransportRequest) -> AuthExtractionResult {
        if !self.config.allow_query_auth || !self.config.supported_methods.contains(&HttpAuthMethod::ApiKeyQuery) {
            return Ok(None);
        }
    ...
    fn enrich_context(&self, mut context: TransportAuthContext, request: &TransportRequest) -> TransportAuthContext {
        // Add client IP
    ...
    fn validate_https(&self, request: &TransportRequest) -> Result<(), TransportAuthError> {
        ...
        let is_https = request.get_header("X-Forwarded-Proto")
            .map(|proto| proto == "https")
            .or_else(|| request.get_header("X-Scheme").map(|scheme| scheme == "https"))
            .or_else(|| request.metadata.get("is_https").map(|_| true))
            .unwrap_or(false);
        
        if !is_https {
            return Err(TransportAuthError::AuthFailed("HTTPS required for authentication".to_string()));
        }
...
impl AuthExtractor for HttpAuthExtractor {
    ...
    async fn validate_auth(&self, context: &TransportAuthContext) -> Result<(), TransportAuthError> {
        // Additional HTTP-specific validation can go here
        if context.credential.is_empty() {
            return Err(TransportAuthError::InvalidFormat("Empty credential".to_string()));
        }
...
mod tests {
    ...
    fn test_bearer_token_extraction() {
        ...
        headers.insert("Authorization".to_string(), "Bearer lmcp_test_1234567890abcdef".to_string());
    ...
    fn test_api_key_header_extraction() {
        let extractor = HttpAuthExtractor::default();
        let mut headers = HashMap::new();
        headers.insert("X-API-Key".to_string(), "lmcp_test_1234567890abcdef".to_string());
    ...
    fn test_query_parameter_extraction() {
        ...
        let request = TransportRequest::new()
            .with_query_param("api_key".to_string(), "lmcp_test_1234567890abcdef".to_string());
