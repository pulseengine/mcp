...
impl StdioAuthExtractor {
    ...
    fn enrich_context(&self, mut context: TransportAuthContext, _request: &TransportRequest) -> TransportAuthContext {
        ...
        if let Ok(cwd) = std::env::current_dir() {
            context = context.with_metadata("working_dir".to_string(), cwd.to_string_lossy().to_string());
        }
...
impl AuthExtractor for StdioAuthExtractor {
    ...
    async fn validate_auth(&self, context: &TransportAuthContext) -> Result<(), TransportAuthError> {
        ...
        if context.method == "Default" {
            tracing::warn!("Using default API key for stdio authentication - not recommended for production");
        }
