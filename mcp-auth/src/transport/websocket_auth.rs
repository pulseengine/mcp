...
impl WebSocketAuthExtractor {
    ...
    fn extract_handshake_headers(&self, headers: &HashMap<String, String>) -> AuthExtractionResult {
        if !self.config.supported_methods.contains(&WebSocketAuthMethod::HandshakeHeaders) {
            return Ok(None);
        }
    ...
    fn extract_query_params(&self, request: &TransportRequest) -> AuthExtractionResult {
        if !self.config.supported_methods.contains(&WebSocketAuthMethod::QueryParams) {
            return Ok(None);
        }
    ...
    fn extract_first_message(&self, request: &TransportRequest) -> AuthExtractionResult {
        if !self.config.supported_methods.contains(&WebSocketAuthMethod::FirstMessage) {
            return Ok(None);
        }
...
mod tests {
    ...
    fn test_subprotocol_extraction() {
        let extractor = WebSocketAuthExtractor::default();
        let mut headers = HashMap::new();
        headers.insert("Sec-WebSocket-Protocol".to_string(), "mcp-auth.lmcp_test_1234567890abcdef".to_string());
        headers.insert("Sec-WebSocket-Key".to_string(), "test-key".to_string());
    ...
    fn test_has_handshake_auth() {
        ...
        let request2 = TransportRequest::new()
            .with_query_param("api_key".to_string(), "token123".to_string());
