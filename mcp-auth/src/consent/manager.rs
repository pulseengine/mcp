...
use super::{ConsentRecord, ConsentError, ConsentType, ConsentStatus, ConsentSummary, LegalBasis, ConsentAuditEntry};
...
impl ConsentManager {
    ...
    pub async fn request_consent(
        ...
        if !self.config.enabled {
            return Err(ConsentError::InvalidData("Consent management is disabled".to_string()));
        }
        
        // Check if consent already exists
        let existing_key = format!("consent:{}:{}", request.subject_id, self.consent_type_key(&request.consent_type));
        if self.storage.get(&existing_key).await.is_ok() {
            return Err(ConsentError::ConsentExists(format!("{}:{:?}", request.subject_id, request.consent_type)));
        }
        ...
        // Set expiration
        if let Some(days) = request.expires_in_days.or(self.config.default_expiration_days) {
            let expires_at = Utc::now() + chrono::Duration::days(days as i64);
            record.set_expiration(expires_at);
        }
        
        // Store consent record
        let consent_data = serde_json::to_string(&record)
            .map_err(ConsentError::SerializationError)?;
        
        self.storage.set(&existing_key, &consent_data).await
        ...
        info!("Consent requested for subject {} with type {:?}", request.subject_id, request.consent_type);
        Ok(record)
    }
    ...
    pub async fn grant_consent(
        ...
        let consent_key = format!("consent:{}:{}", subject_id, self.consent_type_key(consent_type));
        
        // Load existing consent record
        let consent_data = self.storage.get(&consent_key).await
            .map_err(|_| ConsentError::ConsentNotFound(format!("{subject_id}:{consent_type:?}")))?;
        
        let mut record: ConsentRecord = serde_json::from_str(&consent_data)
            .map_err(ConsentError::SerializationError)?;
        ...
        self.storage.set(&consent_key, &updated_data).await
            .map_err(|e| ConsentError::StorageError(e.to_string()))?;
        ...
        info!("Consent granted for subject {} with type {:?}", subject_id, consent_type);
        Ok(record)
    }
    ...
    pub async fn withdraw_consent(
        ...
        warn!("Consent withdrawn for subject {} with type {:?}", subject_id, consent_type);
        Ok(record)
    }
    ...
    pub async fn check_consent(
        ...
        // Try cache first
        {
            ...
            if let Some(record) = cache.values().find(|r| r.subject_id == subject_id && &r.consent_type == consent_type) {
    ...
    pub async fn get_consent_summary(&self, subject_id: &str) -> Result<ConsentSummary, ConsentError> {
        ...
        let is_valid = consents.iter().all(|(_, status)| *status == ConsentStatus::Granted);
    ...
    pub async fn cleanup_expired_consents(&self) -> Result<usize, ConsentError> {
        let cutoff_date = Utc::now() - chrono::Duration::days(self.config.cleanup_expired_after_days as i64);
    ...
    fn consent_type_key(&self, consent_type: &ConsentType) -> String {
        match consent_type {
            ...
            ConsentType::Custom(name) => format!("custom_{}", name.to_lowercase().replace(' ', "_")),
        }
    }
}
...
mod tests {
    ...
    async fn test_consent_request_and_grant() {
        ...
        // Grant consent
        let granted_record = manager.grant_consent(
            "user123",
        ...
        // Check consent
        let is_valid = manager.check_consent("user123", &ConsentType::DataProcessing).await.unwrap();
        assert!(is_valid);
    }
    ...
    async fn test_consent_withdrawal() {
        ...
        // Withdraw consent
        let withdrawn_record = manager.withdraw_consent(
            "user123",
            &ConsentType::Analytics,
            None,
            "test".to_string(),
        ...
        let is_valid = manager.check_consent("user123", &ConsentType::Analytics).await.unwrap();
        assert!(!is_valid);
    }
}