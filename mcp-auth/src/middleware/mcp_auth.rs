...
use crate::{AuthenticationManager, AuthContext, models::Role, security::RequestSecurityValidator};
...
impl McpAuthMiddleware {
    ...
    async fn validate_api_key(
        ...
        match self.auth_manager.validate_api_key(api_key, None).await {
            ...
            Ok(None) => Err(AuthExtractionError::InvalidFormat("Invalid API key".to_string())),
            Err(e) => {
    ...
    async fn check_method_permissions(
        ...
        if let Some(required_roles) = self.config.method_role_requirements.get(method) {
            if let Some(auth_context) = &context.auth.auth_context {
                ...
                let has_required_role = auth_context.roles.iter().any(|role| required_roles.contains(role));
...
mod tests {
    ...
    async fn test_auth_header_parsing() {
        ...
        assert!(matches!(result, Err(AuthExtractionError::UnsupportedMethod(_))));
    }
}