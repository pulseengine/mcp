...
use crate::{
    AuthenticationManager, AuthContext, security::RequestSecurityValidator,
    session::{SessionManager, Session, SessionError}, jwt::JwtError,
    middleware::mcp_auth::{McpAuthConfig, McpRequestContext, AuthExtractionError}
};
use pulseengine_mcp_protocol::{Request, Response, Error as McpError};
...
impl SessionMiddleware {
    ...
    pub async fn process_request(
        ...
        if let Err(security_error) = self.security_validator.validate_request(&request, None).await {
            error!("Request security validation failed: {}", security_error);
            return Err(McpError::invalid_request(&format!("Security validation failed: {}", security_error)));
        }
        ...
        if self.should_skip_auth(&sanitized_request.method) {
            debug!("Skipping authentication for method: {}", sanitized_request.method);
        ...
        match auth_result {
            Ok((auth_context, auth_method, session)) => {
                ...
                if let Some(session) = session {
                    ...
                    match self.create_auto_session(&auth_context, headers).await {
                        Ok(session) => {
                            ...
                            info!("Auto-created session for user: {:?}", auth_context.api_key_id);
                        }
                        Err(e) => {
                ...
                if let Err(e) = self.check_method_permissions(&sanitized_request.method, &base_context).await {
            ...
            Err(e) => {
                if self.config.auth_config.require_auth {
                    ...
                    Err(McpError::invalid_request(&format!("Authentication required: {}", e)))
                } else {
    ...
    async fn authenticate_request(
        ...
        Err(SessionMiddlewareError::AuthError(AuthExtractionError::NoAuth))
    }
    ...
    async fn parse_auth_header(
        ...
        if parts.len() != 2 {
            return Err(SessionMiddlewareError::AuthError(
                AuthExtractionError::InvalidFormat("Invalid Authorization header format".to_string())
            ));
        }
        ...
        match parts[0] {
            ...
            "Basic" => {
                ...
                let decoded = general_purpose::STANDARD.decode(parts[1])
                    .map_err(|_| SessionMiddlewareError::AuthError(
                        AuthExtractionError::InvalidFormat("Invalid Base64 in Basic auth".to_string())
                    ))?;
                
                let decoded_str = String::from_utf8(decoded)
                    .map_err(|_| SessionMiddlewareError::AuthError(
                        AuthExtractionError::InvalidFormat("Invalid UTF-8 in Basic auth".to_string())
                    ))?;
                
                let auth_parts: Vec<&str> = decoded_str.splitn(2, ':').collect();
                if auth_parts.is_empty() {
                    return Err(SessionMiddlewareError::AuthError(
                        AuthExtractionError::InvalidFormat("Basic auth must contain username".to_string())
                    ));
                }
    ...
    async fn validate_api_key(&self, api_key: &str) -> Result<AuthContext, SessionMiddlewareError> {
        let auth_result = self.auth_manager.validate_api_key(api_key, None).await
            .map_err(|e| SessionMiddlewareError::AuthError(
                AuthExtractionError::InvalidFormat(format!("API key validation failed: {}", e))
            ))?;
        
        auth_result.ok_or_else(|| SessionMiddlewareError::AuthError(
            AuthExtractionError::InvalidFormat("Invalid API key".to_string())
        ))
    }
    ...
    async fn create_auto_session(
        ...
        let client_ip = headers
            .and_then(|h| self.config.auth_config.client_ip_header.as_ref().and_then(|ip_header| h.get(ip_header)))
            .cloned();
        ...
        let user_id = auth_context.api_key_id.clone()
            .unwrap_or_else(|| auth_context.user_id.clone().unwrap_or_else(|| "unknown".to_string()));
        
        let (session, _) = self.session_manager.create_session(
            user_id,
            auth_context.clone(),
    ...
    fn should_skip_auth(&self, method: &str) -> bool {
        self.config.auth_config.anonymous_methods.contains(&method.to_string()) ||
        self.config.session_exempt_methods.contains(&method.to_string())
    }
...
mod tests {
    use super::*;
    use crate::{AuthConfig, session::{SessionConfig, MemorySessionStorage}};
    
    async fn create_test_middleware() -> SessionMiddleware {
        let auth_manager = Arc::new(
            crate::AuthenticationManager::new(AuthConfig::default()).await.unwrap()
        );
        let session_manager = Arc::new(
            SessionManager::new(SessionConfig::default(), Arc::new(MemorySessionStorage::new()))
        );
