...
'''
    ...
    async def setup_test_environment(self, server_name: str, repo_url: str) -> Optional[Dict]:
        """Set up a test environment for a server"""
        try:
            # Create temporary directory
            temp_dir = tempfile.mkdtemp(prefix=f"mcp_test_{server_name.replace('/', '_')}_")
    ...
    async def validate_implementation(self, server_name: str, repo_url: str) -> ValidationResult:
    ...
    def generate_report(self, output_file: Optional[str] = None) -> Dict:
        ...
        successful_validations = sum(1 for r in self.results if r.status in ['compliant', 'passed'])
        
        # Calculate statistics
        avg_compliance = None
        if successful_validations > 0:
            scores = [r.compliance_score for r in self.results if r.compliance_score is not None]
            if scores:
        ...
        for result in self.results:
            if result.protocol_version:
                protocol_versions[result.protocol_version] = protocol_versions.get(result.protocol_version, 0) + 1
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        failed_count = sum(1 for r in self.results if r.status not in ['compliant', 'passed'])
        if failed_count > 0:
...
async def main():
    parser = argparse.ArgumentParser(description='Validate MCP servers across the ecosystem')
    ...
    try:
        ...
        if report['summary']['average_compliance_score']:
            print(f"Average compliance: {report['summary']['average_compliance_score']:.1f}%")
