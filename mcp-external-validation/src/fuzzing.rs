...
use crate::{
    ...
    report::{ValidationIssue, IssueSeverity},
};
...
impl McpFuzzer {
    ...
    pub async fn fuzz_server(
        ...
        for i in 0..self.max_iterations {
            ...
            match self.send_fuzzed_input(&client, server_url, &fuzzed_input).await {
                Ok(response) => {
                ...
                Err(e) => {
                    ...
                    if !self.is_duplicate_issue(&result.issues, &issue) {
                        ...
                        let mut issue = issue;
                        issue.reproducible = self.test_reproducibility(
                            &client,
                            server_url,
    ...
    fn generate_fuzzed_input(
        ...
        let json_value = match target {
            FuzzTarget::JsonRpcStructure => {
                let msg = FuzzJsonRpcMessage::arbitrary(&mut u)
                    .map_err(|e| ValidationError::ConfigurationError {
            ...
            FuzzTarget::MethodNames => {
                let method = FuzzMethod::arbitrary(&mut u)
                    .map_err(|e| ValidationError::ConfigurationError {
            ...
            FuzzTarget::ParameterValues => {
                let params = FuzzParams::arbitrary(&mut u)
                    .map_err(|e| ValidationError::ConfigurationError {
    ...
    fn fuzz_message_to_json(&self, msg: &FuzzJsonRpcMessage) -> Value {
        ...
        obj.insert("jsonrpc".to_string(), match &msg.jsonrpc {
            ...
            FuzzJsonRpcVersion::Array(arr) => Value::Array(arr.iter().map(|s| json!(s)).collect()),
            FuzzJsonRpcVersion::Null => Value::Null,
        });
        
        // Add optional fields
        if let Some(method) = &msg.method {
            obj.insert("method".to_string(), json!(self.fuzz_method_to_string(method)));
        }
    ...
    fn fuzz_error_to_json(&self, error: &FuzzError) -> Value {
        ...
        obj.insert("code".to_string(), match &error.code {
    ...
    fn generate_fuzz_uri(&self, rng: &mut fastrand::Rng) -> String {
        match rng.usize(0..15) {
            ...
            12 => format!("resource://{}", std::char::from_u32(rng.u32(..) % 0x110000).unwrap_or('?')),
        ...
        if depth == 0 {
            ...
            match u.int_in_range(0..=1).unwrap_or(0) {
                ...
                _ => {
                    ...
                    let mut obj = serde_json::Map::new();
                    for _ in 0..len {
                        let key = u.arbitrary::<String>().unwrap_or_else(|_| format!("key{}", u.arbitrary::<u32>().unwrap_or(0)));
    ...
    fn is_duplicate_issue(&self, existing: &[FuzzIssue], new: &FuzzIssue) -> bool {
        existing.iter().any(|issue| {
            issue.issue_type == new.issue_type &&
            issue.error == new.error
        })
    }
