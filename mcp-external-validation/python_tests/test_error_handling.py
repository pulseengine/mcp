...
async def test_error_handling(server_url: str, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(server_url, json=invalid_version_request) as response:
                if response.status == 200:
            ...
            async with session.post(server_url, json=missing_method_request) as response:
                if response.status == 200:
                    error_response = await response.json()
                    results["messages_exchanged"] += 2
                    ...
                    if "error" in error_response:
                        error_code = error_response["error"].get("code")
                        if error_code in [-32600, -32602]:  # Invalid Request or Invalid params
                            error_tests_passed += 1
                        else:
                            issues.append({
                                "severity": "warning",
            ...
            async with session.post(server_url, json=unknown_method_request) as response:
                if response.status == 200:
                    error_response = await response.json()
                    results["messages_exchanged"] += 2
            ...
            async with session.post(server_url, json=invalid_params_request) as response:
                if response.status == 200:
                    error_response = await response.json()
                    results["messages_exchanged"] += 2
    ...
    return {
        "success": results["initialized"] and error_tests_passed >= error_tests_total * 0.8,
        "duration_ms": duration_ms,
        "results": results,
        "error": None if results["errors_encountered"] == 0 else "Error handling test failed",
        "issues": issues,
        "compatibility": {
